...
class BuildingDataAnalyzer:
    ...
    def check_tables_exist(self):
        """必要なテーブルが存在するか確認"""
        tables_to_check = [
            'editors', 'building_history', 'admin_boundaries'
        ]
    ...
    def get_building_area_analysis(self) -> pd.DataFrame:
        ...
        if self.debug:
            print(f"✅ Analysis complete for {len(result)} building type/prefecture combinations")

        return result

    def get_3d_data_coverage(self) -> pd.DataFrame:
        """3Dデータカバレッジの計算"""
        query = """
        SELECT
            prefcode,
        ...
        if self.debug:
            print("🔍 Analyzing 3D data coverage...")

        result = self.execute_query(query)

        if self.debug:
            print(f"✅ Found 3D data coverage for {len(result)} prefectures")
    ...
    def analyze_prefecture_building_patterns(self) -> pd.DataFrame:
        ...
        if self.debug:
            print("🔍 Analyzing prefecture building patterns with concentration metrics...")
            ...
            for idx, row in prefecture_data.iterrows():
                ...
                try:
                    if self.debug and idx % 10 == 0:
                        print(f"🔍 Processing prefecture {idx+1}/{len(prefecture_data)}: {prefcode}")
                    ...
                    if not editors_data.empty and editor_count > 0:
                        ...
                        top10pct_edits = editors_data.head(top10pct_count)['edit_count'].sum()
                        ...
                        if self.debug and (idx+1) % 10 == 0:
                            print(f"  {prefcode}: ジニ係数 {gini:.3f}, トップ編集者: {top_editor} ({top_editor_share:.1%})")
                    else:
                        # データがない場合のデフォルト値
                        top_editor = "なし"
                        top_editor_share = 0
                ...
                try:
                    ...
                    if not detail_data.empty:
                        ...
                        # カバレッジ率の計算
                        height_coverage = height_count / building_count * 100 if building_count > 0 else 0
                        levels_coverage = levels_count / building_count * 100 if building_count > 0 else 0
                    else:
                        height_count = levels_count = min_height_count = underground_count = 0
                ...
                # 日付の処理
                first_edit_str = first_edit.strftime('%Y-%m-%d') if first_edit is not None else 'N/A'
                last_edit_str = last_edit.strftime('%Y-%m-%d') if last_edit is not None else 'N/A'
    ...
    def generate_reports(self, output_prefix: str = "building_analysis"):
        ...
        try:
            ...
            output_file = f'{output_dir}/{output_prefix}_building_type_stats_{timestamp}.csv'
            building_type_stats.to_csv(output_file, index=False)
            print(f"✅ Saved to {output_file}")

            # 3. 3Dデータカバレッジ
            print("\n🔍 Analyzing 3D data coverage...")
            coverage_stats = self.get_3d_data_coverage()
            output_file = f'{output_dir}/{output_prefix}_3d_coverage_{timestamp}.csv'
            coverage_stats.to_csv(output_file, index=False)
            print(f"✅ Saved to {output_file}")
            ...
            output_file = f'{output_dir}/{output_prefix}_monthly_activity_{timestamp}.csv'
            ...
            output_file = f'{output_dir}/{output_prefix}_prefecture_analysis_{timestamp}.csv'
            ...
            if not building_stats.empty:
                ...
                print(f"Buildings with height data: {total_height:,} ({total_height/total_buildings*100:.1f}%)")
                print(f"Buildings with levels data: {total_levels:,} ({total_levels/total_buildings*100:.1f}%)")
                print(f"Average building area: {avg_area:.4f} ha")
                print(f"Total building footprint area: {total_area:.4f} ha")

                print("\nTop 5 building types:")
                top5 = building_stats.groupby('building_types').agg({
                    'building_count': 'sum',
                    'avg_height': 'mean',
                    'avg_levels': 'mean',
            ...
            if not prefecture_analysis.empty:
                ...
                if '集中度評価' in prefecture_analysis.columns:
                    concentration_dist = prefecture_analysis['集中度評価'].value_counts()
                ...
                if 'height記述率' in prefecture_analysis.columns:
                    ...
                    display_cols = ['都道府県コード', '建物数', 'height記述数', 'height記述率']
            ...
            if not monthly_activity.empty:
                ...
                print(f"Height data coverage: {latest_month['height_coverage_percent']:.1f}%")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dbname', type=str, default='osm_3ddata_analysis', help='データベース名')
    parser.add_argument('--user', type=str, default='postgres', help='データベースユーザー')
    parser.add_argument('--password', type=str, default='', help='データベースパスワード')
    parser.add_argument('--host', type=str, default='localhost', help='データベースホスト')
    ...
    parser.add_argument('--prefix', type=str, default='building_analysis', help='出力ファイルの接頭辞')
